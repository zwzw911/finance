...
var checkDataTypeBaseOnTypeDefine = function checkDataTypeBaseOnTypeDefine(test) {
    var func = testModule.validateInputValue._private.checkDataTypeBaseOnTypeDefine;
    var value = void 0,
        tmpDataType = void 0,
        result = void 0,
        tmp = void 0;
...
var checkInput = function checkInput(test) {
    var func = testModule.validateInputValue.checkInput;
...
var validateInputSearchFormat = function validateInputSearchFormat(test) {

    test.expect(11);

    var func = testModule.validateInputValue.validateInputSearchFormat;
    var error = miscError.misc.validateInputSearch;
    var rules = require('../../server/define/validateRule/inputRule').inputRule;
    var coll = require('../../server/define/enum/node').node.coll;
    ...
    /*                必须是对象                  */
    value = '1234';
    result = func(value, coll.billType, rules);
    //console.log(result)
    ...
    /*              对象的value为数组，且无undefined/null/''的元素           */
    value = { name: [1, 2, 3, null] };
    result = func(value, coll.billType, rules);
    //console.log(result)
    test.equal(result.rc, error.inputSearchValueStringCanNotEmpty.rc, 'search input value has null element check fail');
    value = { name: [1, 2, 3, ''] };
    result = func(value, coll.billType, rules);
    //console.log(result)
    test.equal(result.rc, error.inputSearchValueStringCanNotEmpty.rc, 'search input value has "" element check fail');

    //成功的输入
    value = { name: [1, 2, 3, 4, 5] };
    result = func(value, coll.billType, rules);
    //console.log(result)
...
var validateInputSearch = function validateInputSearch(test) {

    var func = testModule.validateInputValue.validateInputSearch;
    var error = miscError.misc.validateInputSearch;
    var rules = require('../../server/define/validateRule/inputRule').inputRule;
    var coll = require('../../server/define/enum/node').node.coll;
    // let searchSetting=require('../../server/config/global/globalSettingRule').searchSetting
    ...
    result = func(value, fkAdditionalFieldsConfig, coll.billType, rules);
    test.equal(result.name.rc, rules.billType.name.maxLength.error.rc, 'search input value name a check fail');

    /*    value={'parentBillType':['a']}
        result=func(value,fkAdditionalFieldsConfig,coll.billType,rules)
        // console.log(`result is ${JSON.stringify(result)}`)
        test.equal(result.parentBillType.rc,rules.billType.name.minLength.error.rc,'search input value name a check fail')
    
        value={'parentBillType':['a1234567890a1234567890a1234567890']}
        result=func(value,fkAdditionalFieldsConfig,coll.billType,rules)
        test.equal(result.parentBillType.rc,rules.billType.name.maxLength.error.rc,'search input value name a check fail')*/
...
exports.validate = {
    ...
    validateInputSearch: validateInputSearch
};

//# sourceMappingURL=testMisc_ValidateInputValue-compiled.js.map