...
//根据hashId获得Id
var hashId2Id = function hashId2Id(articleHashId, callback) {
...
var readComment = function readComment(articleHashId, curPage, callback) {
...
var createNewArticle = function createNewArticle(title, authorId, callback) {
    var article = new articleModel();
    article.title = title;
 ...
 * */
var updateArticleContent = function updateArticleContent(articleHashId, obj, callback) {
    //var article=new articleModel();
            ...
            async.forEachOf(article.innerImage, function (value, key, cb) {
                innerImageModel.findById(value, 'hashName', function (err, findedInnerImage) {
                    ...
                    if (null === findedInnerImage) {
                        ...
                        //console.log(idx)
                        if (-1 === idx) {
                            //innerImage中的hashName已经不存在article.htmlContent了
                            var articleInnerImageIdx = article.innerImage.indexOf(value); //重新定位index,因为如果之前删除过,那么index会变化
                            if (-1 !== articleInnerImageIdx) {
                                article.innerImage.splice(articleInnerImageIdx, 1);
                            }
...
//返回一个数组（key id）
var updateArticleKey = function updateArticleKey(articleHashId, keys, callback) {
...
var updateKeyArticle = function updateKeyArticle(articleHashId, callback) {
 ...
 * */
var addInnerImage = function addInnerImage(articleHashID, innerImageModel, callback) {
...
var getAttachmentHashName = function getAttachmentHashName(attachmentId, callback) {
 ...
 * */
var addAttachment = function addAttachment(articleHashID, attachmentModel, callback) {
...
var delAttachment = function delAttachment(articleHashID, attachmentID, callback) {
...
var addComment = function addComment(articleHashID, userId, content, callback) {
...
var delComment = function delComment(articleHashID, commentID, callback) {
...
var userFindById = function userFindById(userId, callback) {
...
var readArticle = function readArticle(articleHashID, callback) {
    //console.log('start')
    ...
    articleModel.find({ hashId: articleHashID }, function (err, doc) {
        ...
        { path: 'innerImage', model: 'innerImages', select: 'name storePath cDate mDate' }, { path: 'attachment', model: 'attachments', select: 'name size cDate', options: { sort: 'cDate' } }];
        //console.log(doc)
