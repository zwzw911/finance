...
//获得关键字的id
var convertKeyword = function convertKeyword(keywordArray, callback) {}
...
;var getSearchResult = function getSearchResult(keywordArray, curPage, callback) {
    ...
    keyModel.aggregate([{ $match: { key: { $in: { $toLower: keywordArray } } } }, { $project: { _id: 1, key: 1 } }, { $group: { _id: null, keys: { $push: "$key" } } }], function (err, res) {
        ...
        articleModel.aggregate([{ $unwind: "$keys" }, //次序会影响结果,要先unwind,然后match
        //{$match:{$or:[{keys:{$in:matchKeyId},state:'编辑完成'}]}},
        { $match: { keys: { $in: matchKeyId } } }, { $group: { _id: "$_id", count: { $sum: 1 } } }, { $sort: { count: -1 } }], function (err, result) {
            ...
            var populateOpt = [{ path: 'author', model: 'users', select: '-_id name' } //对于tree,只要title
...
var getSearchResult1 = function getSearchResult1(keyString, curPage, callback) {
    //keyString='常用';
    var newCurPage = curPage; //不知为何，function的参数surPage无法传递到articleModel.aggregate内部
    var searchOpt = [{ $match: { $text: { $search: keyString } } }, { $sort: { rank: { $meta: "textScore" } } }, { $project: { "_id": 1 } } //为了populate使人民居住作者名字，所以只能先获得article id
    ];
