...
//判断是否为目录的创建者
var ifFolderOwner = function ifFolderOwner(userId, folderId, callback) {
...
var readRootFolderId = function readRootFolderId(userId, folderName, callback) {
...
var ifDefaultFolder = function ifDefaultFolder(folder) {
...
var checkIfRootFolder = function checkIfRootFolder(folderId, callback) {
 ...
 * */
var createRootFolder = function createRootFolder(userId, folderName, callback) {
    var defaultFolderName = general.defaultRootFolderName;
    if (-1 === defaultFolderName.indexOf(folderName)) {
        return callback(null, runtimeNodeError.folder.invalidateRootFolderName);
    }
...
* */
var readRootFolder = function readRootFolder(userId, folderName, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
    var defaultFolderName = general.defaultRootFolderName;
...
//读取用户指定目录下的目录信息
var readFolder = function readFolder(userId, parentFolderId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
...
* newName: folder的新名字
* */
var modifyFolderName = function modifyFolderName(userId, folderId, oldName, newName, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
 ...
 * */
var moveFolder = function moveFolder(userId, folderId, oldParentFolderId, newParentFolderId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
    if (!validateFolder._id.type.define.test(folderId)) {
...
* */
var createNewFolder = function createNewFolder(userId, parentFolderId, newFolderName, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
    if (!validateFolder._id.type.define.test(parentFolderId)) {
...
* folder:要删除的目录
* */
var deleteFolder = function deleteFolder(userId, folderId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
    if (!validateFolder._id.type.define.test(folderId)) {
...
//统计当前目录下子目录的数量
var countSubFolder = function countSubFolder(userId, parentId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
...
/*********************************************************************************************************************/
//读取目录下所有文档的数量
var readArticleNumInFolder = function readArticleNumInFolder(folderId, callback) {
 ...
 * */
var readArticleInFolder = function readArticleInFolder(userId, folderId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
    ...
    var opt = [{ path: 'articleId', model: 'articles', select: 'title state author hashId mDate' } //对于tree,只要title
...
var readArticleInFolderForPagination = function readArticleInFolderForPagination(userId, folderId, curPage, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
    if (!validateArticleFolder.folderId.type.define.test(folderId)) {
    ...
    ifFolderOwner(userId, folderId, function (err, result) {
        ...
        countSubArticle(userId, folderId, function (err, result1) {
            ...
            articleFolderModel.find({ folderId: folderId }, 'articleId', { limit: general.articleFolderPageSize, skip: (paginationInfo.curPage - 1) * general.articleFolderPageSize }, function (err, articleFolder) {
                ...
                var opt = [{ path: 'articleId', model: 'articles', select: 'title author cDate mDate' } //对于tree,只要title
                //{path:'comment',model:'comments',select:'content mDate user',options:{limit:general.commentPageSize}}
                ];
                for (var i = 0; i < totalNum; i++) {
                    articleFolder[i].populate(opt, function (err, populatedArticle) {
...
*  folderId:文档要加入(移入)的目录编号
* */
var createArticleFolder = function createArticleFolder(userId, articleId, folderId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
...
* */
var removeArticleFolder = function removeArticleFolder(userId, articleId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
    if (!validateArticleFolder.articleId.type.define.test(articleId)) {
 ...
 * */
var moveArticle = function moveArticle(userId, articleHashId, oldFolderId, newFolderId, callback) {
    if (!validateFolder.owner.type.define.test(userId)) {
        return callback(null, validateFolder.owner.type.client);
    }
...
//统计当前目录下文档的数量
var countSubArticle = function countSubArticle(userId, folderId, callback) {
    if (!validateFolder._id.type.define.test(folderId)) {
        return callback(null, validateArticleFolder.folderId.type.client);
    }
    ifFolderOwner(userId, folderId, function (err, result) {
...
//# sourceMappingURL=personalArticle-compiled.js.map