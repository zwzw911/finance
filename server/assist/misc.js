...
//    注意，返回的结果是对象，结构和inputValue类似，不是{rc;xxx,msg:xxx}的格式
    ...
    *           返回{rc:0,msg:'xxxx}
    * */
    validateInputSearchFormat:function(inputSearch,fkAdditionalFieldsConfig,collName,inputRules) {
...
// console.log(`non fk field value is ${singleSearchElement}`)
    ...
    * */
    validateInputSearch:function(inputSearch,fkAdditionalFieldsConfig,collName,inputRules){
        let result={}
...
/*                    //如果是外键，找到外键所在coll的对应字段的定义
    ...
    checkSingleSearchValue:function(chineseName,singleSearchString,singleFieldRule){
...
/*                result[singleFieldName]['rc']=currentRule['error']['rc']
        ...
        let typeCheckResult = validateInputValue._private.checkDataTypeBaseOnTypeDefine(singleSearchString,singleFieldRule['type'])
...
// console.log(`currentRule is ${JSON.stringify(currentRule)},currentRuleDefine is ${currentRuleDefine}`)
            switch (singleRule){
                case 'min':
                    if(true===validateInputValue._private.ruleTypeCheck.exceedMin(singleSearchString,currentRuleDefine)){
                        result['rc']=currentRule['error']['rc']
                        result['msg']=validateInputValue._private.generateErrorMsg.min(chineseName,currentRuleDefine,false,currentRule['unit'])
                        return result
/*                        result[singleFieldName]['rc']=currentRule['error']['rc']
                    ...
                    break;
                case 'max':
                    if(true===validateInputValue._private.ruleTypeCheck.exceedMax(singleSearchString,currentRuleDefine)){
                        result['rc']=currentRule['error']['rc']
                        result['msg']=validateInputValue._private.generateErrorMsg.max(chineseName,currentRuleDefine,false,currentRule['unit'])
                        return result
                        // result[singleFieldName]['rc']=currentRule['error']['rc']
                    ...
                    break;
                case 'minLength':
                    if(true===validateInputValue._private.ruleTypeCheck.exceedMinLength(singleSearchString,currentRuleDefine)){
                        result['rc']=currentRule['error']['rc']
                        result['msg']=validateInputValue._private.generateErrorMsg.minLength(chineseName,currentRuleDefine,false)
                        return result
                        // result[singleFieldName]['rc']=currentRule['error']['rc']
                ...
                case 'maxLength':
                    ...
                    if(true===validateInputValue._private.ruleTypeCheck.exceedMaxLength(singleSearchString,currentRuleDefine)){
                        result['rc']=currentRule['error']['rc']
                        result['msg']=validateInputValue._private.generateErrorMsg.maxLength(chineseName,currentRuleDefine,false)
                        return result
                        // result[singleFieldName]['rc']=currentRule['error']['rc']
