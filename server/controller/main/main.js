...
router.delete("/",function(req,res,next){
    if('development'===envSetting){
        unifiedRouterController.removeAll().then(
...
/*router.post('/create',function(req,res,next){
    unifiedRouterController.create(req).then(
        function(v){
            console.log(`create ${JSON.stringify(req.body.values[validatePart.currentColl])} success, result:  ${JSON.stringify(v)}`)
            return res.json(v)
        }
    ).catch(
        function(err){
            console.log(`create ${JSON.stringify(req.body.values[validatePart.currentColl])} fail: ${JSON.stringify(err)}`)
            return res.json(err)
        }
    )
})*/
router.post('/create',function(req,res,next){
    unifiedRouterController.create(req).then(
        ...
        (err)=>{
            // console.log(`create ${JSON.stringify(req.body.values[validatePart.currentColl])} fail: ${JSON.stringify(err)}`)
            console.log(`create  fail: ${JSON.stringify(err)}`)
            return res.json(err)
...
router.put('/update',function(req,res,next){
    unifiedRouterController.update(req).then(
        (v)=>{
            // console.log(`update ${req.body.values[validatePart.currentColl]} success, result: ${JSON.stringify(v)}`)
            return res.json(v)
        },
        (e)=>{
            // console.log(`update ${req.body.values[validatePart.currentColl]} fail: ${JSON.stringify(e)}`)
            return res.json(e)
        }
    )
})
...
router.post('/search',function(req,res,next){
    ...
    unifiedRouterController.search(req).then(
        (v)=>{
            // console.log(`search ${req.body.values[validatePart.currentColl]} success,result: ${JSON.stringify(v)}`)
            return res.json(v)
        },
        (e)=>{
            // console.log(`search ${req.body.values[validatePart.currentColl]} fail: ${JSON.stringify(e)}`)
            return res.json(e)
        })
})

//searchParams（用来确定删除后分页）已经id都放在post的body中
router.post('/delete',function(req,res,next){
    //console.log(req.params)
    unifiedRouterController.remove(req).then(
        (v)=>{
            // console.log(`delete ${req.body.values[validatePart.currentColl]} success, result: ${JSON.stringify(v)}`)
            return res.json(v)
        },
        (e)=>{
            // console.log(`delete ${req.body.values[validatePart.currentColl]} fail: ${JSON.stringify(e)}`)
            return res.json(e)
        })
})

//read single field
router.post('/filterFieldValue',function(req,res,next){
    // console.log('get no name ')
    unifiedRouterController.filterFieldValue(req).then(
        (v)=>{
            console.log(`read all ${req.body.values[validatePart.currentColl]} name success, result: ${JSON.stringify(v)}`)
            return res.json(v)
        },
        (e)=>{
            console.log(`read all ${req.body.values[validatePart.currentColl]} name fail: ${JSON.stringify(e)}`)
            return res.json(e)
        })
})
