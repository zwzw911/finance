...
app.factory('financeCRUDHelper', function ($http, $q, inputAttrHelper, commonHelper, cont) {
    ...
    var _angularDataOp = {
        ...
        //recorder: 数组，当前所有记录
        'create': function create(singleReturnRecorder, inputAttr, recorder) {
        ...
        'update': function update(idx, inputAttr, recorder) {
        ...
        'read': function read(newGetRecorder, recorder) {}
    };

    var dataOperator = {
        billType: {
            //idx无用，只是为了统一使用参数(create和update同样在modal上操作，使用同一个按钮)
            'create': function create(idx, inputAttr, recorder, selectAC) {
            ...
            'update': function update(idx, inputAttr, recorder, selectAC) {
            ...
            'readName': function readName(name, suggest_state) {
                return $http.get('/billType/name/' + name, {});
            }
        },
        department: {
            //idx无用，只是为了统一使用参数
            'create': function create(idx, inputAttr, recorder) {
                //首先加入db
    ...
    */
    ...
    var suggest_state = {
        ...
        employee: {
            name: function name(term) {
                ...
                for (var i = 0; i < states.length && results.length < 10; i++) {
                    var state = states[i];
                    if (state.toLowerCase().indexOf(q) === 0) results.push({ label: state, value: state });
                }
                return results;
            }
        },
    ...
    return { dataOperator: dataOperator, suggest_state: suggest_state };
});
